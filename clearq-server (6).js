...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      const slotDur = s?.slot_duration_mins || 30;

      // Pending = booked but not started yet (these are in the queue)
      const pending = await db(
        `SELECT id FROM wash_bookings WHERE shop_id=$1 AND status='pending' AND kind IN ('reservation','walkin')`,
        [shopId]
      );

      // Active washes currently in progress (real washes, not maintenance)
      ...
      // Occupied bays = unique bays with any in_progress booking (including maintenance)
      const occupiedBays = await db(
        `SELECT COUNT(DISTINCT bay_number) as cnt FROM wash_bookings WHERE shop_id=$1 AND status='in_progress' AND bay_number IS NOT NULL`,
        [shopId]
      );
      const occupiedCount = parseInt(occupiedBays[0]?.cnt || 0);
      const freeBays = Math.max(0, maxBays - occupiedCount);

      const queueCount = pending.length;
      const activeCount = active.length;

      // Wait time logic:
      // All bookings (active + pending) occupy bay slots in order.
      // We simulate a timeline of bay availability and find when the
      // LAST booking finishes — that is the wait for a new customer.
      //
      // Each active wash has a known eta_ready_at.
      // Each pending wash has eta_arrival_at + duration.
      // We assign each pending wash to the soonest free bay slot.

      const nowMs = Date.now();

      // Get full details for active and pending bookings
      const activeDetail = await db(
        `SELECT eta_ready_at, wash_started_at, eta_arrival_at FROM wash_bookings 
         WHERE shop_id=$1 AND status='in_progress' AND kind IN ('reservation','walkin')`,
        [shopId]
      );
      const pendingDetail = await db(
        `SELECT eta_arrival_at, eta_ready_at FROM wash_bookings 
         WHERE shop_id=$1 AND status='pending' AND kind IN ('reservation','walkin') 
         ORDER BY created_at ASC`,
        [shopId]
      );

      // Build array of when each bay will next be free (in ms)
      // Start with active washes
      let bayFreeAt = [];
      for (const b of activeDetail) {
        const freeAt = b.eta_ready_at
          ? new Date(b.eta_ready_at).getTime()
          : b.wash_started_at
            ? new Date(b.wash_started_at).getTime() + slotDur * 60000
            : nowMs + slotDur * 60000;
        bayFreeAt.push(freeAt);
      }
      // Fill remaining free bays with nowMs
      while (bayFreeAt.length < maxBays) bayFreeAt.push(nowMs);
      bayFreeAt.sort((a, b) => a - b);

      // Simulate assigning each pending booking to the soonest free bay
      let lastFinishAt = nowMs;
      for (const b of pendingDetail) {
        // This booking starts when either: the bay is free OR they arrive (whichever is later)
        const arrivalAt = b.eta_arrival_at
          ? new Date(b.eta_arrival_at).getTime()
          : nowMs + slotDur * 60000;
        const readyAt = b.eta_ready_at
          ? new Date(b.eta_ready_at).getTime()
          : null;

        // Assign to soonest free bay
        bayFreeAt.sort((a, b) => a - b);
        const soonestBay = bayFreeAt[0];
        const startAt = Math.max(soonestBay, arrivalAt);
        const finishAt = readyAt || (startAt + slotDur * 60000);
        bayFreeAt[0] = finishAt;
        if (finishAt > lastFinishAt) lastFinishAt = finishAt;
      }

      // Wait time for a NEW customer = when the last current booking finishes
      const totalBookings = activeDetail.length + pendingDetail.length;
      let waitMins = 0;
      if (totalBookings > 0 && lastFinishAt > nowMs) {
        waitMins = Math.max(1, Math.round((lastFinishAt - nowMs) / 60000));
      }
