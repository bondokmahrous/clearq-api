...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      // Simulate bay slots — start all bays as free now
      let bays = Array(maxBays).fill(nowMs);

      // Fill active washes first (they already have real finish times)
      ...
      // Now assign each pending booking to the next available bay slot
      for (const b of pending) {
        bays.sort((a, x) => a - x);
        const dur = parseInt(b.duration_mins) || defaultDur;
        const bayFreeAt = bays[0];
        // Customer arrives at eta_arrival_at or right now if walk-in/no ETA
        const arriveAt = b.eta_arrival_at
          ? new Date(b.eta_arrival_at).getTime()
          : nowMs;
        // Wash starts when bay is free AND customer has arrived
        const startAt = Math.max(bayFreeAt, arriveAt);
        const finishAt = startAt + dur * 60000;
        bays[0] = finishAt;
      }

      // Wait for a NEW customer = when the last busy bay finishes
