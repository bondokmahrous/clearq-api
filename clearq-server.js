...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      // BAY SLOT SIMULATION:
      // Assign every booking (active + pending) to bays in order.
      // Each booking takes its service duration in a bay.
      // A new customer enters the soonest free bay after all current bookings.
      let bays = Array(maxBays).fill(nowMs);

      // Active washes — already in a bay, finish at eta_ready_at
      ...
      // Pending bookings — assign to soonest free bay after arrival
      ...
      // New customer enters the SOONEST free bay after all bookings
      bays.sort((a, x) => a - x);
      const nextFree = bays[0]; // soonest bay free for a new customer
