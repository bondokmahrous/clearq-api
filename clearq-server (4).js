...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      // - If there are free bays and no queue: 0 wait
      // - If queue exists: (position in queue) * slotDur / freeBays
      // - If all bays full: time until soonest bay frees up
      let minsUntilNextFree = 0;

      if (freeBays === 0 && active.length > 0) {
        // All bays full — find soonest one to free
        ...
        minsUntilNextFree = Math.max(1, Math.round((soonest - nowMs) / 60000));
      }

      // Total wait for next person = time until a bay is free + queue ahead * slot duration / bays
      const effectiveBays = Math.max(1, maxBays);
      const queueWait = queueCount > 0
        ? minsUntilNextFree + Math.ceil(queueCount * slotDur / effectiveBays)
        : minsUntilNextFree;
