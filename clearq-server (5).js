...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      // Wait time logic (simple and correct):
      // - Free bay available → 0 wait, next booking goes straight in
      // - All bays full → wait = time until soonest bay done
      //   + (any remaining queue beyond that) × slotDur / maxBays
      let waitMins = 0;

      if (freeBays === 0) {
        // All bays occupied — find when the soonest one finishes
        const nowMs = Date.now();
        // Sort active washes by eta so we can assign queue slots
        const etaTimes = active.map(b => {
          if (b.eta_ready_at) return new Date(b.eta_ready_at).getTime();
          if (b.wash_started_at) return new Date(b.wash_started_at).getTime() + slotDur * 60000;
          return nowMs + slotDur * 60000;
        }).sort((a, b) => a - b);

        if (etaTimes.length > 0) {
          // First pending booking waits until soonest bay is done
          const soonestFree = etaTimes[0];
          waitMins = Math.max(1, Math.round((soonestFree - nowMs) / 60000));

          // If there are MORE pending bookings beyond the first,
          // each additional one adds slotDur/maxBays extra wait
          if (queueCount > 1) {
            waitMins += Math.ceil((queueCount - 1) * slotDur / Math.max(1, maxBays));
          }
        }
      }
      // If freeBays > 0: wait = 0 regardless of pending count (they go straight to a free bay)
