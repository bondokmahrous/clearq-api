...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      // SIMPLE QUEUE LOGIC:
      // Every booking (pending or active) will take slotDur minutes in a bay.
      // New customer waits until ALL current bookings are done.
      // We simulate filling N bays with bookings in order.
      const nowMs = Date.now();

      // Get every booking that is not yet completed
      const allBookings = await db(
        `SELECT status, eta_ready_at, wash_started_at, eta_arrival_at, created_at
         FROM wash_bookings
         WHERE shop_id=$1 AND status IN ('pending','in_progress') AND kind IN ('reservation','walkin')
         ORDER BY created_at ASC`,
        [shopId]
      );

      // Start each bay as free right now
      let bays = Array(maxBays).fill(nowMs);

      for (const b of allBookings) {
        // When does this booking finish?
        let finishAt;
        if (b.status === 'in_progress') {
          // Already started — use eta_ready_at or started_at + slotDur
          finishAt = b.eta_ready_at
            ? new Date(b.eta_ready_at).getTime()
            : new Date(b.wash_started_at || b.created_at).getTime() + slotDur * 60000;
        } else {
          // Pending — use eta_ready_at if available (most accurate),
          // otherwise simulate: arrives in slotDur mins, wash takes slotDur
          if (b.eta_ready_at) {
            finishAt = new Date(b.eta_ready_at).getTime();
          } else {
            bays.sort((a, b) => a - b);
            ...
            finishAt = startAt + slotDur * 60000;
          }
        }
      ...
      // If there are free bays right now, wait = 0 (go straight in)
      // Only show wait if ALL bays are currently occupied
      let waitMins = 0;
      if (freeBays === 0) {
        const lastFinish = Math.max(...bays);
        waitMins = lastFinish > nowMs
