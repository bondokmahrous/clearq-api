...
async function route(req, res) {
  ...
  if (pages[p]) {
    // Look in same directory first, then templates subdirectory
    let file = path.join(__dirname, pages[p]);
    if (!fs.existsSync(file)) file = path.join(__dirname, "templates", pages[p]);
    try {
      const html = fs.readFileSync(file, "utf-8");
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8", ...CORS_HEADERS });
      return res.end(html);
    } catch {
      return respond(res, 404, { error: "Page not found" });
    }
  }
  ...
  try {
    ...
    // GET /wash/queue/:shopId — FIXED
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      const shopId = +p.split("/")[3];
      const t = today();
      const s = await db1(`SELECT * FROM wash_shops WHERE id=$1`, [shopId]);
      const maxBays = s?.max_workers || 3;
      const slotDur = s?.slot_duration_mins || 45;

      const pending = await db(
        `SELECT id FROM wash_bookings WHERE shop_id=$1 AND scheduled_date=$2 AND status='pending' AND kind IN ('reservation','walkin')`,
        [shopId, t]
      );
      const active = await db(
        `SELECT eta_ready_at, wash_started_at FROM wash_bookings WHERE shop_id=$1 AND scheduled_date=$2 AND status='in_progress' AND kind IN ('reservation','walkin')`,
        [shopId, t]
      );
      ...
      if (activeCount >= maxBays && active.length > 0) {
        const nowMs = Date.now();
        let soonest = Infinity;
        for (const b of active) {
          let freeAt;
          if (b.eta_ready_at) {
            freeAt = new Date(b.eta_ready_at).getTime();
          } else if (b.wash_started_at) {
            freeAt = new Date(b.wash_started_at).getTime() + slotDur * 60000;
          } else {
            freeAt = nowMs + slotDur * 60000;
          }
          if (freeAt < soonest) soonest = freeAt;
        }
        minsUntilNextFree = soonest === Infinity
          ? slotDur
          : Math.max(1, Math.round((soonest - nowMs) / 60000));
      }

      return respond(res, 200, { queueCount, activeCount, maxBays, minsUntilNextFree });
    }
