...
async function route(req, res) {
  ...
  try {
    ...
    if (m === "GET" && /^\/wash\/queue\/\d+$/.test(p)) {
      ...
      // Simple correct queue logic:
      // All bookings (pending + active) occupy bay slots.
      // Wait time for a NEW customer = when the last slot finishes.
      const nowMs = Date.now();

      // Get all current bookings with their finish times
      const allActive = await db(
        `SELECT eta_ready_at, wash_started_at FROM wash_bookings 
         WHERE shop_id=$1 AND status='in_progress' AND kind IN ('reservation','walkin')`,
        [shopId]
      );
      const allPending = await db(
        `SELECT eta_ready_at, eta_arrival_at FROM wash_bookings 
         WHERE shop_id=$1 AND status='pending' AND kind IN ('reservation','walkin')
         ORDER BY created_at ASC`,
        [shopId]
      );

      // Build bay slots — each slot = when that bay will next be free
      let baySlots = [];
      // Active washes occupy bays now
      for (const b of allActive) {
        const finishAt = b.eta_ready_at
          ...
          : b.wash_started_at
            ...
            : nowMs + slotDur * 60000;
        baySlots.push(finishAt);
      }
      // Fill remaining free bays
      while (baySlots.length < maxBays) baySlots.push(nowMs);
      baySlots.sort((a, b) => a - b);

      // Assign each pending booking to the soonest free bay
      for (const b of allPending) {
        baySlots.sort((a, b) => a - b);
        const soonestBay = baySlots[0];
        // Booking starts when bay is free OR customer arrives (whichever is later)
        const arrivalAt = b.eta_arrival_at
          ? new Date(b.eta_arrival_at).getTime()
          : nowMs + slotDur * 60000;
        const startAt = Math.max(soonestBay, arrivalAt);
        // Finish = use eta_ready_at if available, else start + slotDur
        const finishAt = b.eta_ready_at
          ? new Date(b.eta_ready_at).getTime()
          : startAt + slotDur * 60000;
        baySlots[0] = finishAt;
      }

      // Wait for a NEW customer = when the busiest bay finishes
      baySlots.sort((a, b) => a - b);
      const lastFinish = baySlots[baySlots.length - 1];
      const waitMins = lastFinish > nowMs
        ? Math.max(1, Math.round((lastFinish - nowMs) / 60000))
        : 0;
